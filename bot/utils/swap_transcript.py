"""
Swap Ticket Transcript Generator
Creates beautiful HTML transcripts of swap ticket conversations
"""

import discord
from datetime import datetime
from typing import List
from utils.support_transcript import parse_discord_content, get_logo_base64


def generate_swap_transcript_html(
    swap_id: str,
    ...
    """Generate professional HTML transcript for a swap ticket with Afroo theme"""
    ...
    from_asset = swap_data.get("from_asset", "N/A")
    to_asset = swap_data.get("to_asset", "N/A")
    input_amount = swap_data.get("input_amount", 0)
    actual_output = swap_data.get("actual_output", swap_data.get("estimated_output", 0))
    ...
    created_at = swap_data.get("created_at", datetime.utcnow())
    ...
    deposit_display = changenow_deposit_address if len(changenow_deposit_address) <= 30 else f"{changenow_deposit_address[:15]}...{changenow_deposit_address[-10:]}"
    dest_display = destination_address if len(destination_address) <= 30 else f"{destination_address[:15]}...{destination_address[-10:]}"
    hash_display = payout_hash if len(payout_hash) <= 30 else f"{payout_hash[:15]}...{payout_hash[-10:]}"

    # Build full HTML with Afroo theme (using same CSS as support transcripts)
    html = f'''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Swap Transcript - {swap_id[:8]} - Afroo Exchange</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <!-- Sticky Navigation -->
    <div class="top-nav">
        <div class="nav-title">Swap Transcript - {swap_id[:8]}</div>
    ...
    <div class="main-layout">
        ...
        <div class="content-area">
            ...
            <div class="header-section">
                {logo_html}
                <div class="header-title">Swap Transcript</div>
                <div class="header-subtitle">Official Document</div>
                <div class="ticket-badge">{from_asset} → {to_asset}</div>
            </div>
            ...
            <div class="footer-section">
                ...
                <p class="footer-text">Professional Cryptocurrency Swap Services</p>
        ...
        <div class="sidebar">
            <div class="sidebar-card">
                <div class="sidebar-title">Swap Information</div>
                <div class="metadata-item">
                    <span class="metadata-label">Swap ID</span>
                    <span class="metadata-value">{swap_id[:8]}</span>
                </div>
                <div class="metadata-item">
                ...
                <div class="metadata-item">
                    <span class="metadata-label">From Asset</span>
                    <span class="metadata-value">{from_asset}</span>
                </div>
                <div class="metadata-item">
                    <span class="metadata-label">To Asset</span>
                    <span class="metadata-value">{to_asset}</span>
                </div>
                <div class="metadata-item">
                    <span class="metadata-label">Input Amount</span>
                    <span class="metadata-value">{input_amount} {from_asset}</span>
                </div>
                <div class="metadata-item">
                    <span class="metadata-label">Output Amount</span>
                    <span class="metadata-value">{actual_output:.8f} {to_asset}</span>
                </div>
                <div class="metadata-item">
                    <span class="metadata-label">Exchange Rate</span>
                    <span class="metadata-value">1 {from_asset} = {exchange_rate:.8f} {to_asset}</span>
                </div>
            </div>

            <div class="sidebar-card">
                <div class="sidebar-title">Addresses</div>
                <div class="metadata-item">
                    <span class="metadata-label">Deposit Address</span>
                    <span class="metadata-value">{deposit_display}</span>
                </div>
                <div class="metadata-item">
                    <span class="metadata-label">Receiving Address</span>
                    <span class="metadata-value">{dest_display}</span>
                </div>
                <div class="metadata-item">
                    <span class="metadata-label">Transaction Hash</span>
                    <span class="metadata-value">{hash_display}</span>
                </div>
            </div>
            ...
            <div class="sidebar-card">
                ...
                <div class="metadata-item">
                    <span class="metadata-label">Initiated By</span>
                    <span class="metadata-value">{opened_by.name}</span>
                </div>
                <div class="metadata-item">
                    <span class="metadata-label">Created At</span>
                    <span class="metadata-value">{created_at.strftime("%B %d, %Y")}</span>
                    <span class="metadata-value" style="font-size: 12px; opacity: 0.7;">{created_at.strftime("%I:%M %p UTC")}</span>
                </div>
                <div class="metadata-item">
